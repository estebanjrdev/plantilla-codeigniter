...
class Ion_auth_model extends CI_Model
{
	...
	public function delete_group($group_id = FALSE)
	{
        ...
        $id = $group_id;
       $this->db->query("UPDATE `users_groups` SET `group_id`='2' WHERE `group_id`='".$id."'");
	...
	public function editar_user_Roll($data){
	$this->db->set('group_id', $data['rol'] );
	
	$this->db->where('user_id=' . $data['id']);
	
	if ($this->db->update('users_groups')) {
	return TRUE;
	}
	return FALSE;
	}
